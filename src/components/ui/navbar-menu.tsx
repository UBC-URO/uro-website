                    ...
                    {active === item && (
                        <div className="absolute top-[calc(100%_+_0.6rem)] left-1/2 transform -translate-x-1/2 pt-4">
                            <motion.div
                                ...
                                className="bg-white backdrop-blur-sm rounded-2xl overflow-hidden border border-black/[0.2] shadow-sm"
                            >
                                <motion.div
...
export const Menu = ({ className, setActive, children }: { className?: string; setActive: (item: string | null) => void; children: React.ReactNode }) => {
    return (
        <nav
            onMouseLeave={() => setActive(null)} // resets the state
            className={cn("relative rounded-full border border-transparent shadow-input flex justify-center space-x-6 py-4", className)}
        >
            {children}
        </nav>
    );
